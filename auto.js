...
bro.describe("equalities", (_) => {
  ...
  bro.test("deep equal", (_) => {
    ...
    bro
      .expect(new Date("2011-03-31").equals(new Date("2011-03-31")))
      .toBe(true);
    bro
      .expect(new Date("2011-03-31").equals(new Date("1970-01-01")))
      .toBe(false);
...
bro.describe("object matching", (_) => {
  bro.test("match", (_) => {
    ...
    bro
      .expect(
        matches(
          { l: 1, c: 3, tl: null, tc: null },
          { l: 1, c: 3, tl: null, tc: null, hc: 3 },
        ),
      )
      .toBe(true);
  });

  //bro.test("simple rough matches", () => {
  //  bro.expect({ x: 3.0005 }).toRoughlyMatch({ x: 3.0, y: 3 }, 1e-3);
  //  bro
  //    .expect(new DOMPoint(1, 2))
  //    .toRoughlyMatch(new DOMPoint(1, 2.00005), 1e-3);
  //});
});
...
bro.describe("expectations", (_) => {
  bro.test("to match object", (_) => {
    ...
    bro
      .expect(() => new Expectation({ a: 2, b: 3 }).toMatchObject({ a: 1 }))
      .toThrow();
    bro
      .expect(() =>
        new Expectation({ a: 2, b: 3 }).toMatchObject({ a: 2, b: 3, c: 4 }),
      )
      .toThrow();
    bro
      .expect(() => new Expectation({ a: 2, b: 3 }).toMatchObject({ C: 1 }))
      .toThrow();
  });
});
